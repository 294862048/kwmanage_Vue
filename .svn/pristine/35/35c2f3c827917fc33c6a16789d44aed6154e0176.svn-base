<template>
  <div>
    ...
    <el-table ref="lutiTable"
              ...
              :height="tableHeight"
              border
              :data="DoneLuruDataList.slice((currentPage-1)*pageSize, currentPage * pageSize)"
              tooltip-effect="dark"
  ...
  export default {
    ...
    watch: {
      ...
      searchTableInfo () {
        ...
        if (searchTableInfo) {
          this.DoneLuruDataList = this.DoneLuruDataList.filter(value => value.luti_code.indexOf(this.searchTableInfo) !== -1)
        } else {
    ...
    methods: {
      ...
      getDoneLutiByLururenId () {
        var getData = this;
        this.$axios.get('/luti/getDoneLutiByLururenId', {
          params: {
            lutistate_lururenid: sessionStorage.getItem("user_id")
          }
        })
          .then((res) => {
            getData.DoneLuruDataList = res.data.result;
          })
          .catch((err) => {
            console.log(err);
            alert('获取信息失败！');

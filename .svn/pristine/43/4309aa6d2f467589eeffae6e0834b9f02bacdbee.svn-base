<template>
  <div>
    <div style="text-align: left; margin: 5px">
      ...
      <el-input clearable prefix-icon="el-icon-search" v-model="searchTableInfo" placeholder="请输入搜索内容" style="width:240px"></el-input>
    </div>
  ...
  export default {
    ...
    watch: {
      ...
      searchTableInfo(){
        ...
        var template = []
        if(searchTableInfo){
          template1 = this.jiaoshiData.filter(value => value.jiaoshi_xingming.indexOf(this.searchTableInfo) !== -1)
          template2 = this.jiaoshiData.filter(value => value.xuexiao_mingcheng.indexOf(this.searchTableInfo) !== -1)
          //合并去重复
          let json = template1.concat(template2)
          for (let i = 0; i < json.length ; i++) {
            let flag = true;  //建立标记，判断数据是否重复，true为不重复
            for (let j = i+1; j < json.length ; j++) {
              if(json[i].jiaoshi_id == json[j].jiaoshi_id){ //让json数组对象的内容与新数组的内容作比较，相同的话，改变标记为false
                flag = false;
              }
            }
            if(flag){ //判断是否重复
              template.push(json[i]); //不重复的放入新数组。  新数组的内容会继续进行上边的循环。
            }
          }
          this.jiaoshiData = template;
        } else {
          this.getAllJiaoshi();
        }
      },

...
<!--    学期信息表格-->
    <el-table :default-sort = "{prop: 'xueqi_mingcheng'}"
              ...
              ref="xueqiTable"
              border
              :data="xueqiData.slice((currentPage-1)*pageSize,currentPage*pageSize)"
              tooltip-effect="dark"
              style="width: auto">
      <!--多选框-->
  ...
  export default {
    ...
    watch: {
      ...
      searchTableInfo(){
        ...
        if(searchTableInfo){
          this.xueqiData = this.xueqiData.filter(value => value.xueqi_mingcheng.indexOf(this.searchTableInfo) !== -1)
        } else {
          this.getAllXueqi();
        }
      },

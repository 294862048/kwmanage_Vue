<template>
  <div>
    ...
    <el-table :default-sort = "{prop: 'kemu_mingcheng', order: 'descending'}"
              ...
              ref="kemuTable"
              border
              :data="kemuData.slice((currentPage-1)*pageSize,currentPage*pageSize)"
              tooltip-effect="dark"
              style="width: auto"
  ...
  export default {
    ...
    watch: {
      ...
      searchTableInfo(){
        ...
        if(searchTableInfo){
          this.kemuData = this.kemuData.filter(value => value.kemu_mingcheng.indexOf(this.searchTableInfo) !== -1)
        } else {
          this.getAllKemu();
        }
      },
